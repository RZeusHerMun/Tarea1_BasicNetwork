...
class Network(object): #Definimos nuestra red como una clase en la que definimos 

    def __init__(self, sizes): #Definimos los atributos de la clase: numero de neuronas por capas, bias y weights.
    ...
    def feedforward(self, a): # Esta función nos da el valor de inicialización 'a' determinandolo a partir de la f. sigmoide.
        for b, w in zip(self.biases, self.weights):
            a = sigmoid(np.dot(w, a)+b)
        return a

'''
La siguiente función es el Stochastic Gradient Descent; recordando que basa su función en 
obtener los weights adecuados con el back propagation, ademas de que calcula solo una derivada 
por minibatch
'''
    def SGD(self, training_data, epochs, mini_batch_size, eta,
            test_data=None):# definimos las variables a usar (los datos de entrenamiento, las epocas, el tamano de los batchs y el LearningRate)
    ...
    def update_mini_batch(self, mini_batch, eta): # Aqui definimos la actualizacion de los bias y los weights de los minibatchs
    
        nabla_b = [np.zeros(b.shape) for b in self.biases]
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        '''
        Usamos dos listas para almacenar las sumas de los gradientes de cada minibatch. Son del mismo tamano que las de las bias y los weights.
        '''
        for x, y in mini_batch:
            ...
            '''
            En este ciclo usamos la funcion del BackPropagation definida mas abajo en la que calculamos los gradientes de C (la funcion de coste)
            con los bias y weights 'originales'.
            Despues sumamos a los gradientes definimos (los nabla)
            '''
        ...
        """
        Cuando se calculan todos los gradientes del minibatch, se actualizan los bias y los weights con la resta del aprendizaje.
        """
    
    
    def backprop(self, x, y): #Codigo del Algoritmo BackPropagation
        ...
        delta = self.cost_derivative(activations[-1], y) * \
            sigmoid_prime(zs[-1]) # una vez tenemos la lsita de las activaciones, sacamos el error de la ultima capa.
        nabla_b[-1] = delta
        nabla_w[-1] = np.dot(delta, activations[-2].transpose()) #guardamos los erroores en las listas "nablas" de atras hacia adelante.
        ...
        for l in range(2, self.num_layers): #ahora usamos los errores para calcular los gradientes de la ultima capa.
            ...
            nabla_b[-l] = delta 
            nabla_w[-l] = np.dot(delta, activations[-l-1].transpose()) # usamos el error para calcular los gradientes de la capa y los vamos guardando en las listas de "nablas".
        return (nabla_b, nabla_w) # al final las listas ya tiene los gradientes de cada capa de la red.

    def evaluate(self, test_data): # En esta función evaluamos los datos de prueba que clasifico la red.
    ...
    def cost_derivative(self, output_activations, y): # Definimos la funcion de costo con la derivada respecto a las activaciones de salida. 
        return (output_activations-y)



def sigmoid(z): # Esta es la funcion sigmoide que usaremos
    return 1.0/(1.0+np.exp(-z))

def sigmoid_prime(z): # Y aqui­ definimos la derivada de la funcion sigmoide.
