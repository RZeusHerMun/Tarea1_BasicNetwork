...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        '''Voy a tratar de hacer un graficador del aprendizaje de la red neuronal'''
        # Las listas para almacenar el rendimiento
        ...
        for j in range(epochs):# Para el numero de epocas q definamos se entrenara como sigue.
            ...
            training_accuracy = self.evaluate(test_data) / n_test
            training_performance.append(training_accuracy)
            print(f"Epoch {j}: Training Accuracy {training_accuracy}") # FUNCIONANDO AL CIEN
            
            if test_data:
                plt.plot(range(epochs), training_performance, label='Training Accuracy')
                plt.plot(range(epochs), test_performance, label='Test Accuracy')
                plt.xlabel('Epocas')
